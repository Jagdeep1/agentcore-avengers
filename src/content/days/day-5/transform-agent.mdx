...
# Code Interpreter is available through strands-tools when running in AgentCore

# Import pattern for AgentCore Code Interpreter:
try:
...
# Import Code Interpreter from strands-tools (available in AgentCore Runtime)
try:
    from strands_tools import code_interpreter
...
# Build tools list (conditionally include code_interpreter if available)
tools = [
